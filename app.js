const APP_VERSION = "4-consolidated-builds";
...
function renderPartsList(items) {
  ...
  items.forEach((entry) => {
    ...
    li.innerHTML = `<strong>${entry.part}:</strong> ${entry.value}. ${entry.why}`;
    ul.appendChild(li);
  });
  return ul;
}
...
function createTierCard(tier, pathType, mode) {
  ...
  if (alternative) {
    ...
    altUl.className = "parts-list";
    alternative.parts.forEach(({ part, value }) => {
      const li = document.createElement("li");
      li.innerHTML = `<strong>${part}:</strong> ${value}`;
